...
ContextUitl 可以创建 Context ，在创建 Context 时要指定 Context 的入口 EntranceNode，同时根据 EntranceNode 创建调用链的第一个 Entry ，经过上面的唠叨你应该知道 EntranceNode 必然是第一层节点。ContextUtil 会将所有第一层节点存储在 contextNameNodeMap 中，所有请求在构建 Entry调用链 时都必须先从 contextNameNodeMap 取，看看对应的第一层节点是否已经创建好了，这样可以避免重复创建。

由于并发问题，这里的代码会是你第一次看到 Sentinel 使用 双重检查锁 机制，其实在 Sentinel 中，双重检查锁用的特别多。
...
所以在写代码的时候 contextNameNodeMap 中就已经有了一个 EntranceNode 了。当然这是题外话了，我们要说的是 Context 的创建。
...
protected static Context trueEnter(String name, String origin) {
    ...
    if (context == null) {
        ...
        // 上面的逻辑是创建ROOT下的第一层节点，不管需不需要创建 EntranceNode ，反正Context 是一定要的。
...
- ROOT 并不参与 Context 的构建，也不参与Entry的执行。Context 的 EntranceNode 都是 第一层节点。

- Entry 是调用链，第一层节点，也就是 EntranceNode，一定是所有 Entry 调用链中第一个 Entry。

  但是你会发现这些节点其实并没有啥功能需要做，那么这些 EntranceNode 会执行处理器链吗？留一个疑点，下一章揭晓。
