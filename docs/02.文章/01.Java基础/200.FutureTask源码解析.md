...
- Callable.call() 方法结束，将运行的结果交给 FutureTask，FutureTask 停止阻塞。
...
// COMPLETING 完成状态， 表示 FutureTask 中的 callable 已经执行完了，但是结果还没有赋值给 result
...
如果 state > COMPLETING 就说明任务已经执行完了，不管是正常执行还是异常执行，反正现在可以把结果result返回给主线程。
...
由于有多个线程，所以要有一个变量表示开启 callable.call() 的线程 runner，其他线程都只执行了 futureTask.get()，只有这个 runner 线程是执行 futureTask.run() 开启 callable.run() 的。

如果有多个线程调用 futureTask.get() ，而 callable.call() 恰好在阻塞，那么这些线程都会被封装为链表的形式，等到 callable 执行结束会挨个唤醒。
...
作为链表节点 WaitNode，其实 FutureTask 中并没有实现其他内容去维护这个链表，比如并发安全的插入删除操作，而是简单的使用 Unsafe 去执行将节点添加到链表中的操作。
...
有了 Unsafe，可以执行 CAS 的操作去将 WaitNode 挂在 waiters 前面, 也就是头插法。
...
如果让咱们借助上面的变量实现 run 方法，你会如何实现？

首先要做判断，可能有多个线程调用了 FutureTask.run()但是只有一个可以成功调用，如何判断？用 state 变量和 runner 变量.

- 如果 state 不为 NEW，说明 callable 已经被其他线程执行了。
- 如果通过 CAS 无法将 runner 从 null 改为本线程，说明 callable 已经被其他线程执行了。
...
try {
    ...
    // 再次判断
    if (c != null && state == NEW) {
        // result结果变量，将来设置给 outcome 的。
        V result;
        // 是否正常运行，正常运行完成可以将执行结果赋值给 outcome，运行异常就要将 exception 赋值给 outcome
        boolean ran;
        try {
            // 开始执行
            result = c.call();
...
protected void set(V v) {
    // 使用 CAS 将 state 从 NEW 改为 COMPLETING
    if (UNSAFE.compareAndSwapInt(this, stateOffset, NEW, COMPLETING)) {
        // 将结果赋值给outcome
        outcome = v;
        // 使用 CAS 将 state 改为 NORMAL
        UNSAFE.putOrderedInt(this, stateOffset, NORMAL); 
        // 遍历 waitter，唤醒所有阻塞的线程
        finishCompletion();
    }
}
```
...
private void finishCompletion() {
	// 为什么要双重循环，其实只是判断一下 waiters 是否为空
...
public void run() {
    ...
    try {
        ...
        if (c != null && state == NEW) {
            ...
            // 如果ran为true, 可以做四件事:
            // 1. 将状态改为 COMPLETING，如果成功可执行下面的步骤
            // 2. 将结果赋值给 outcome
            // 3. 将状态改为 Normal
            // 4. 唤醒所有阻塞的线程
            if (ran)
